...
`Input gain -> dry tap -> one-sample feedback FET gain-reduction loop -> FET/transformer character stage -> optional auto-gain compensation on wet path -> parallel mix -> output gain`.
...
Auto gain is a post-detection wet-path compensation stage. It follows the current gain-reduction amount, adds a capped amount of makeup to the compressed path, and does not feed the compensated signal back into the FET detector.

The Deluxe detector mode is interpreted as `Vintage` or `Fast`. Both modes remain peak-led; `Fast` uses a stronger peak blend and quicker transient capture. The character stage provides `Transformer` and `FET Push` drive colors using asymmetric tanh shaping after gain reduction, with a small low-band emphasis and high-band shave before the nonlinear stage. The selectable `2x` and `4x` oversampling modes run the nonlinear character stage through JUCE half-band polyphase IIR up/downsampling instead of the earlier interpolation-only path. Output gain is applied after the feedback sidechain tap, so it does not increase compression amount. This is still an inspired digital model, not a component-level recreation of a specific hardware unit.
...
Current 76-style ranges: input -24 to +24 dB; hidden FET calibration -60 to 0 dB; ratio buttons 4:1, 8:1, 12:1, 20:1, and ALL; attack 0.02 to 0.8 ms; release 50 to 1100 ms; output -24 to +24 dB; mix 0 to 100%; hidden output trim -24 to +12 dB; knee 0 to 8 dB; sidechain HPF 20 to 500 Hz; auto gain on/off; oversampling Off, 2x, and 4x.

## Factory Presets

The processor exposes factory presets through the JUCE program API and the bottom-rail preset selector. Current presets are `76 Start`, `Vocal Grab`, `Bass Hold`, `Drum Smash`, `Parallel Snap`, and `Mix Kiss`. Presets are starting points only: the saved plugin state stores the selected program index and all parameter values, so edited preset settings round-trip with the session.
...
Open DSP work: tune attack/release/character against reference material. Open product work: screenshot/release-note capture and host validation at minimum/default/maximum editor sizes.
